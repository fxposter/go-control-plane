...
type watches struct {
	mu                sync.Mutex
	responsesInFlight []cache.Response

	responseNotifications chan struct{}
...
func (values *watches) Init() {
	values.responseNotifications = make(chan struct{}, 1)
	values.responses = make(chan cache.Response)
...
func (s *server) process(stream Stream, reqCh <-chan *discovery.DiscoveryRequest, defaultTypeURL string) error {
	...
	go func() {
		for resp := range values.responses {
			values.mu.Lock()
			values.responsesInFlight = append(values.responsesInFlight, resp)
			values.mu.Unlock()

			select {
			case values.responseNotifications <- struct{}{}:
			default:
			}
		}
	}()
	...
	for {
		...
		case <-values.responseNotifications:
			values.mu.Lock()
			responses := values.responsesInFlight
			values.responsesInFlight = nil
			values.mu.Unlock()
