...
func (cache *snapshotCache) CreateWatch(request *Request, cb func(Response)) func() {
	...
	if !exists || request.VersionInfo == version {
		...
		info.mu.Lock()
		info.watches[watchID] = ResponseWatch{Request: request, Response: cb}
		info.mu.Unlock()
		return cache.cancelWatch(nodeID, watchID)
	}

	// otherwise, the watch may be responded immediately
	cache.respond(request, cb, snapshot.GetResources(request.TypeUrl), version)
...
func (cache *snapshotCache) respond(request *Request, cb func(Response), resources map[string]types.Resource, version string) {
	...
	cb(createResponse(request, resources, version))
}
